...
plugin_path = resolveFilename(SCOPE_PLUGINS, "Extensions/nssaddon/")
data_path = os.path.join(plugin_path, 'lib/')
...
config.plugins.nssaddon.hostaddress = NoSave(ConfigText(default='127.0.0.1'))
config.plugins.nssaddon.port = NoSave(ConfigNumber(default=15000))
config.plugins.nssaddon.user = NoSave(ConfigText(default='Enter Username', visible_width=50, fixed_size=False))
config.plugins.nssaddon.passw = NoSave(ConfigPassword(default='******', fixed_size=False, censor='*'))

# ===================================================
host = str(config.plugins.nssaddon.hostaddress.value)
port = str(config.plugins.nssaddon.port.value)
user = str(config.plugins.nssaddon.user.value)
password = str(config.plugins.nssaddon.passw.value)
...
def putlblcfg():
    ...
    global buttn
    global putlbl
    putlbl = config.plugins.nssaddon.cfgfile.getValue()
    buttn = ''
...
class cccConfig(Screen, ConfigListScreen):
        ...
        if config.plugins.nssaddon.active.value is True:
            ...
            try:
                ...
                if 'oscam' in res.lower() or 'icam' in res.lower() or 'ncam' in res.lower() or 'gcam' in res.lower():
                    ...
                    msg = []
                    msg.append(_("\n....\n.....\n"))
                    self.cmd1 = '/usr/lib/enigma2/python/Plugins/Extensions/nssaddon/lib/emm_sender.sh'
    ...
    def createSetup(self):
        ...
        if config.plugins.nssaddon.active.getValue():
            ...
            self.list.append(getConfigListEntry(_('Server Port'), config.plugins.nssaddon.port, _('Port')))
            self.list.append(getConfigListEntry(_('Server Username'), config.plugins.nssaddon.user, _('Username')))
            self.list.append(getConfigListEntry(_('Server Password'), config.plugins.nssaddon.passw, _('Password')))
    ...
    def CCcam(self):
        ...
        if config.plugins.nssaddon.cfgfile.value != '/etc/CCcam.cfg':
            self.session.open(MessageBox, _('Select CCcam'), type=MessageBox.TYPE_INFO, timeout=5)
            return
        cfgfile = config.plugins.nssaddon.cfgfile.value
        dest = cfgfile
        host = 'C: ' + str(config.plugins.nssaddon.hostaddress.value)
        port = str(config.plugins.nssaddon.port.value)
        user = str(config.plugins.nssaddon.user.value)
        pasw = str(config.plugins.nssaddon.passw.value)
    ...
    def Ncam(self):
        global host, port, user, passw
        if config.plugins.nssaddon.cfgfile.value != '/etc/tuxbox/config/ncam.server':
    ...
    def getcl(self):
        try:
            data1 = str(config.plugins.nssaddon.Server.value)
            print(data1)
            ...
            if url1 != '':
                ...
                if 'cccameurop' in data.lower():
                    ...
                    for h, p, u, pw in url1:
                        ...
                        password = password.replace('</div>', '')
                # if config.plugins.nssaddon.active.getValue():
                config.plugins.nssaddon.hostaddress.setValue(host)
                config.plugins.nssaddon.port.setValue(port)
                config.plugins.nssaddon.user.setValue(user)
                config.plugins.nssaddon.passw.setValue(password)
                self.createSetup()
            else:
                return
        except Exception as e:
